...
const withExpoAutolinking = (config) => {
  return withSettingsGradle(config, (config) => {
    ...
    const oldPattern = /extensions\.configure\(com\.facebook\.react\.ReactSettingsExtension\) \{ ex ->\s*if \(System\.getenv\('EXPO_USE_COMMUNITY_AUTOLINKING'\) == '1'\) \{\s*ex\.autolinkLibrariesFromCommand\(\)\s*\} else \{\s*def command = \[/g;
